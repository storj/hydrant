...
func NewHandler(b *backend.Backend, mem *MemStore, source *FixedSource, cfg *config.Config) http.Handler {
	...
	return hmux.Dir{
		...
		"/api": hmux.Dir{
			"/query": hmux.Method{
				"GET": http.HandlerFunc(h.queryHandler),
			},
			"/keys": hmux.Method{
				"GET": http.HandlerFunc(h.keysHandler),
			},
			"/values": hmux.Method{
				"GET": http.HandlerFunc(h.valuesHandler),
			},
			...
			"/config": hmux.Method{
				...
				"POST": http.HandlerFunc(h.configHandlerPOST),
			},
			"/collect": hmux.Method{
				"POST": protocol.NewHTTPHandler(b),
			},
		},
	}
}
...
func (h *handler) queryHandler(w http.ResponseWriter, r *http.Request) {
	...
	if m {
		...
		if err := h.mem.Query(q, func(name []byte, hist *flathist.Histogram) bool {
...
func (h *handler) valuesHandler(w http.ResponseWriter, r *http.Request) {
	ctx := hydrant.WithSubmitter(r.Context(), h.b)
	...
	h.mem.KeyValues(key, func(value []byte) bool {
...
func (h *handler) annotationsHandler(w http.ResponseWriter, r *http.Request) {
	...
	h.mem.Annotations(func(annotation []byte) bool {
...
func (h *handler) serverConfigHandlerPOST(w http.ResponseWriter, r *http.Request) {
	...
	if err := dec.Decode(&dests); err != nil {
		http.Error(w, err.Error(), http.StatusBadRequest)
		return
	}
