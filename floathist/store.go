...
type Store struct {
	...
	l0  arena[layer0]
	l1  arena[layer1]
	l2s arena[layer2Small]
	l2l arena[layer2Large]
...
// Handle is a handle to a histogram.
type Handle struct {
	v arenaP[layer0]
}

// New allocates a new histogram and returns a handle.
func (s *Store) New() Handle { return Handle{v: s.l0.New()} }

func (s *Store) getL0(h Handle) *layer0       { return s.l0.Get(h.v) }
func (s *Store) getL1(v uint32) *layer1       { return s.l1.Get(arenaRaw[layer1](v & lAddrMask)) }
func (s *Store) getL2S(v uint32) *layer2Small { return s.l2s.Get(arenaRaw[layer2Small](v & lAddrMask)) }
func (s *Store) getL2L(v uint32) *layer2Large { return s.l2l.Get(arenaRaw[layer2Large](v & lAddrMask)) }
...
// either g or h.
func Merge(s *Store, g Handle, t *Store, h Handle) {
	...
	for bm := new32(bitmask(&hl0.l1)); !bm.Empty(); bm.ClearLowest() {
		l1idx := bm.Lowest()
		...
		for bm := new32(bitmask(&hl1.l2)); !bm.Empty(); bm.ClearLowest() {
			l2idx := bm.Lowest()
...
func (s *Store) Observe(h Handle, v float32) {
...
func (s *Store) Min(h Handle) float32 {
	l0 := s.l0.Get(h.v)

	i := uint32(new32(bitmask(&l0.l1)).Lowest())
	...
	j := uint32(new32(bitmask(&l1.l2)).Lowest())
...
//
// It is safe to be called concurrently with Observe.
func (s *Store) Max(h Handle) float32 {
	l0 := s.l0.Get(h.v)

	i := uint32(new32(bitmask(&l0.l1)).Highest())
	l1a := atomic.LoadUint32(&l0.l1[i])
	if l1a == 0 {
		return float32(math.NaN())
	}
	...
	j := uint32(new32(bitmask(&l1.l2)).Highest())
	l2a := atomic.LoadUint32(&l1.l2[j])
	if l2a == 0 {
		return float32(math.NaN())
	}
...
func (s *Store) Reset(h Handle) {
	l0 := s.l0.Get(h.v)
	for bm := new32(bitmask(&l0.l1)); !bm.Empty(); bm.ClearLowest() {
		l1 := s.getL1(l0.l1[bm.Lowest()])

		for bm := new32(bitmask(&l1.l2)); !bm.Empty(); bm.ClearLowest() {
			l2a := l1.l2[bm.Lowest()]
...
//
// It is safe to be called concurrently with Observe.
func (s *Store) Total(h Handle) (total uint64) {
	l0 := s.l0.Get(h.v)
	for bm := new32(bitmask(&l0.l1)); !bm.Empty(); bm.ClearLowest() {
...
//
// It is safe to be called concurrently with Observe.
func (s *Store) Quantile(h Handle, q float64) (v float32) {
...
//
// It is safe to be called concurrently with Observe.
func (s *Store) CDF(h Handle, v float32) float64 {
...
//
// It is safe to be called concurrently with Observe.
func (s *Store) Summary(h Handle) (total, sum, avg, vari float64) {
	var total2 float64
...
//
// It is safe to be called concurrently with Observe.
func (s *Store) Distribution(h Handle, cb func(value float32, count, total uint64)) {
